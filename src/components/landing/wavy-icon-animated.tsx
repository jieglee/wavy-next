"use client";

import { motion } from "motion/react";
import { useEffect } from "react";

const WAVY_PATH =
    "M429.246 276.556c32.29-2.874 57.828 3.454 79.292 14.497c9.163 4.37-1.33 14.103-14.366 7.894c-27.778-9.957-34.29-14.33-65.843-12.865c-9.417-.113-8.475-8.533.917-9.526m-125.798-10.808c-3.908-8.232-18.616-15.07-32.297.983c0 0 7.417-75.495 7.697-82.778c3.462-16.378-16.136-11.32-17.772-9.945c-35.778 23.668-76.65 157.81-76.65 157.81c4.386-62.402 19.779-143.033 30.396-221.867c.798-7.797-13.233-4.015-13.233-4.015c-61.955 22.094-75.166 183.334-90.402 183.998c-4.393-45.143-1.51-51.918-17.772-50.657c-21.91 5.137-15.144 13.582-43.935 38.983c-17.1-5.867-43.194-3.452-43.194-3.452c-8.569 1.538-8.192 8.984 0 9.382c14.218-.84 39.286 2.522 52.099 2.662c16.016-.52 20.19-13.432 26.049-13.867c5.832 67.65 5.52 57.392 22.591 58.126c34.005-7.975 47.835-111.503 76.736-179.62c-16.606 92.058-30.233 184.025-28.8 243.958c.34 14.236 8.074 11.67 18.053 10.739c17.505-1.634 46.822-113.569 77.525-180.745c-2.2 17.509-7.469 82.746-7.469 82.746c1 6.959 21.586 5.36 23.526.886l17.695-28.293c15.959 28.721 17.832 25.35 28.275 25.35c10.523 0 21.28-16.944 21.28-16.944c3.068 4.176 4.792 8.88 6.38 9.883c2.75 2.713 15.659 2.538 19.342.575c0 0 18.85-11.582 33.383-13.447c19.528-1.657 15.278-10.672 0-11.298c-10.511-.513-34.458 7.796-34.458 7.796c-7.737-21.282-3.62-21.247-19.173-21.803c-11.598-.28-26.425 22.176-26.425 22.176s-6.071-10.212-9.447-17.322";

type WavyIconAnimatedProps = {
    size?: number;
    color?: string;
    className?: string;
    /** Durasi satu siklus "gambar" (detik). Siklus penuh (gambar+tahan+hapus) otomatis lebih panjang dari ini. */
    drawDuration?: number;
    /** Kalau true, animasi loop terus. Kalau false, cuma jalan sekali. */
    loop?: boolean;
    /** Dipanggil SEKALI aja, pas gambar pertama kali selesai — dipakai buat trigger elemen lain (misal nama brand). */
    onFirstDrawComplete?: () => void;
};

export function WavyIconAnimated({
    size = 48,
    color = "currentColor",
    className,
    drawDuration = 1.1,
    loop = true,
    onFirstDrawComplete,
}: WavyIconAnimatedProps) {
    // Dipisah dari onAnimationComplete bawaan Framer Motion, karena itu bakal kepanggil TIAP siklus loop.
    // Kita cuma mau trigger sekali, jadi pakai timer manual sesuai durasi gambar pertama.
    useEffect(() => {
        if (!onFirstDrawComplete) return;
        const timer = setTimeout(onFirstDrawComplete, drawDuration * 1000);
        return () => clearTimeout(timer);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    return (
        <svg
            width={size}
            height={size}
            viewBox="0 0 512 512"
            role="img"
            aria-label="Wavy logo"
            className={className}
        >
            {/* Ghost layer: siluet logo transparan tipis, selalu ada biar bentuknya gak "hilang total" pas fase hapus */}
            <path d={WAVY_PATH} fill={color} opacity={0.12} />

            {/* Garis yang terus bergerak: gambar → tahan sebentar → hapus → ulang */}
            <motion.path
                d={WAVY_PATH}
                fill="none"
                stroke={color}
                strokeWidth={8}
                strokeLinecap="round"
                strokeLinejoin="round"
                initial={{ pathLength: 0 }}
                animate={loop ? { pathLength: [0, 1, 1, 0] } : { pathLength: 1 }}
                transition={
                    loop
                        ? {
                            duration: drawDuration * 2.4,
                            times: [0, 0.35, 0.75, 1],
                            repeat: Infinity,
                            repeatDelay: 0.6,
                            ease: "easeInOut",
                        }
                        : { duration: drawDuration, ease: "easeInOut" }
                }
            />
        </svg>
    );
}