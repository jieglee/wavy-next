...
export default function ConcertOrganizerShare({ eventTitle }: { eventTitle?: string }) {
  const url = getUrl();
  ...
  return (
    <section className="rounded-2xl border border-[#E5E7EB] bg-white p-5 sm:p-6">
      ...
      <p className="mt-1 text-xs text-[#6B7280]">Ajak teman nonton bareng.</p>
      <div className="mt-4 flex items-center gap-2">
        ...
        <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}`} target="_blank" rel="noopener noreferrer" className="flex h-10 w-10 items-center justify-center rounded-full bg-[#0A66C2] text-white" aria-label="LinkedIn">
