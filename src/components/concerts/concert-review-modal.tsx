...
export default function ConcertReviewModal({ rating, setRating, comment, setComment, submitting, onClose, onSubmit }: Props) {
  return (
      ...
      <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
        ...
        <form onSubmit={onSubmit} className="mt-4 space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            <button type="button" onClick={onClose} className="flex-1 rounded-xl border border-[#EDEBF2] py-2.5 text-xs font-bold text-[#6B6875] hover:bg-gray-50">
              Batal
            </button>
            <button type="submit" disabled={submitting} className="flex-1 rounded-xl bg-[#1B1A3A] py-2.5 text-xs font-bold text-white hover:bg-black">
